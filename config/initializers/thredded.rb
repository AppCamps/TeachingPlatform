...
# By default, thredded uses a simple permission model, where all the users can post to all message
# boards, and admins and moderators are determined by a flag on the users table.
...
# :position            (default)  set the position manually (new messageboards go to the bottom,
...
Thredded.email_from = "no-reply@#{fetch_from_env('DOMAIN')}"

# Emails going out will prefix the "Subject:" with the following string
Thredded.email_outgoing_prefix = '[App Camps Community]'
#
...
# Thredded.slugifier =
#   ->(input) { Babosa::Identifier.new(input).normalize.transliterate(:russian).to_s }
...
#   Thredded::ContentFormatter.after_markup_filters.index(HTML::Pipeline::EmojiFilter)
# ] = MyEmojiFilter
...
# ==> Error Handling
# By default Thredded just renders a flash alert on errors such as Topic not found, or Login
# required. Below is an example of overriding the default behavior on LoginRequired:
#
...
#     # This is called in the Thredded view context, so all Thredded helpers and URLs are accessible
# here directly.
#     'hi'
#   end
# end
...
# ==> Topic following
#
# By default, a user will be subscribed to a topic they've created. Uncomment this to not subscribe
# them:
#
# Thredded.auto_follow_when_creating_topic = false
#
# By default, a user will be subscribed to (follow) a topic they post in. Uncomment this to not
# subscribe them:
#
...
# `follow_topics_on_mention` column
# of the `thredded_user_preferences` and `thredded_user_messageboard_preferences` tables.

# ==> Notifiers
#
# Change how users can choose to be notified, by adding notifiers here, or removing the initializer
# altogether
#
# default:
...
# Thredded.notifiers = [
#   Thredded::EmailNotifier.new, Thredded::PushoverNotifier.new(ENV['PUSHOVER_APP_ID'])
# ]

Thredded::ApplicationController.module_eval do
  before_action :authenticate_user!

  attr_accessor :prev_locale
  before_action :set_german_locale
  after_action :reset_locale

  private

  def set_german_locale
    self.prev_locale = I18n.locale
    I18n.locale = :de
  end
