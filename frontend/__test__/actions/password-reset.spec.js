...
describe('PasswordReset actions', () => {
  describe('passwordResetRequest', () => {
    it("should send create request to api, show success notification and push('/')", (done) => {
      const passwordResetRequestApiCallMock = jest.fn(() => (Promise.resolve({})));
      ...
      return store
               ...
               .then(() => {
                 ...
                 expect(passwordResetRequestApiCallMock.mock.calls[0]).to.deep.eql([formData]);
                 done();
               }).catch(done.fail);
    });
  });
  ...
  describe('passwordReset', () => {
    describe('success', () => {
      it("should send create request to api, show success notification and push('/')", (done) => {
        ...
        const formData = { password: 'asdasdasd', passwordConfirmation: 'asdasdasd' };
        ...
        return store
                 ...
                 .then(() => {
                   ...
                   expect(passwordResetApiCallMock.mock.calls[0]).to.deep.eql([formData, token]);
                   done();
                 });
      });
      ...
      describe('failure', () => {
        it("should show invalid token error notification and push('/password-reset') if resetPasswordToken is invalid", (done) => {
          const passwordResetApiCallMock = jest.fn(() =>
            Promise.reject(new SubmissionError({ resetPasswordToken: 'is invalid.' })),
          );
          RewireAPI.__Rewire__(
            'passwordResetApiCall',
            passwordResetApiCallMock,
        ...
        it("should show expired token error notification and push('/password-reset') if resetPasswordToken is invalid", (done) => {
          const passwordResetApiCallMock = jest.fn(() =>
            Promise.reject(new SubmissionError({ resetPasswordToken: 'has expired, please request a new one' })),
          );
          RewireAPI.__Rewire__(
            'passwordResetApiCall',
            passwordResetApiCallMock,
        ...
        it('should rethrow submission error for other errors', (done) => {
          const thrownError = new SubmissionError({ passwordConfirmation: 'notEqual.' });
          const passwordResetApiCallMock = jest.fn(() =>
