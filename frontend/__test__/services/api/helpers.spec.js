...
import { fetch, initializeApi, __RewireAPI__ as ApiRewire } from '../../../services/api/helpers';
    ...
    it('dispatches notification server network', (done) => {
      ...
      fetch('/api/server-error', { method: 'GET' })
        .catch(() => {
          ...
          expect(logMock.mock.calls[0]).to.eql(['GET /api/server-error: [Request]']);

          const trackErrorCalls = trackErrorMock.mock.calls;
          expect(trackErrorCalls).to.have.length(1);
          expect(trackErrorCalls[0][0].message).to.eql('Request failed with status code 500');
