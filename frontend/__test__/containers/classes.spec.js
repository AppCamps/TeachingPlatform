  ...
  describe('mapStateToProps', () => {
    ...
    describe('classes', () => {
      it('includes courses and courseSchoolClass in classes', () => {
        const { session: { Course }, factory, state } = store;
        const klass = factory.create('schoolClass', {}, { coursesCount: 0 });
        const courses = factory.createList('course', 2, {}, { lessonsCount: 0 });
        ...
        const orderedCoursesRef = Course.all()
          ...
          .map((course) => {
            ...
            course.includeRef.courseSchoolClass = course.courseSchoolClasses.first().includeRef;
      ...
      it('includes lessons with expertises in courses', () => {
        const { session: { Lesson }, factory, state } = store;
        const klass = factory.create('schoolClass', {}, { coursesCount: 0 });
        const course = factory.create('course', { class: klass }, { lessonsCount: 0 });
        const lessons = factory.createList('lesson', 2, { course }, { expertisesCount: 3 });
      ...
      it('includes continueLesson in course', () => {
        ...
        const klass = factory.create('schoolClass', {}, { coursesCount: 0 });
        const course = factory.create('course', { class: klass }, { lessonsCount: 0 });
        const lessons = factory.createList('lesson', 2, { course }, { expertisesCount: 0 });

        klass.update({ courses: [course.id], completedLessons: [lessons[0].id] });
        course.update({ lessons: lessons.map(c => c.id) });
