...
import { mapStateToProps, mapDispatchToProps, __RewireAPI__ as containerRewire } from '../../../containers/login/locality';
...
describe('Classes Container', () => {
  describe('mapStateToProps', () => {
    ...
    it('returns the user', () => {
      ...
      expect(user).to.deep.eql({ ...User.last().includeRef, isPersisted: false });
    });

    it('returns all countries and countryOptions', () => {
      const { session: { Country }, factory, state } = store;
    ...
    it('returns stateOptions for selectedCountry', () => {
      ...
      props.stateOptions.forEach((option, index) => {
        ...
        expect(option).to.deep.equal({ value: stateKey, label: states[stateKey] });
      });
    });

    it('upcases stateOptions for string values', () => {
      const { session: { Country }, factory, state } = store;
      factory.createList('country', 1, {
  ...
  describe('mapDispatchToProps', () => {
    it('createLocality', () => {
      ...
      containerRewire.__Rewire__('createInitialLocality', createInitialLocality);
