...
import { LocalStorage, CookieStorage, MemoryStorage, getStorage } from '../../utils/storage';
...
describe('Storage', () => {
  describe('Fallback mechanism', () => {
    it('should use local storage when supported', () => {
      ...
      expect(storage)
        .to.exist
        .and.be.instanceof(LocalStorage);
    });
    ...
    it('should fallback to cookies when local storage is not supported', () => {
      ...
      expect(storage)
        .to.exist
        .and.be.instanceof(CookieStorage);
    });
    ...
    it('should fallback to memory storage when local storage is not supported and cookies are disabled', () => {
      ...
      expect(storage)
        .to.exist
        .and.be.instanceof(MemoryStorage);
    });
  });
