...
describe('<Lesson/>', () => {
  ...
  it('should contain headings and description', () => {
    ...
    expect(wrapper.containsMatchingElement(<h2>{course.topic.title}</h2>)).to.eql(true);
    expect(wrapper.containsMatchingElement(<h3>{`${course.title} - ${lesson.title}`}</h3>)).to.eql(true);
    expect(wrapper).to.contain(<div className={style.lessonDescription}>{reactParser(lesson.description)}</div>);
  });
  ...
  it('should include course progress in title if given', () => {
    ...
    const wrapper = mount(<Lesson {...defaultProps} courseProgress={params} />, { context: { t } });
    ...
    expect(wrapper.find('h1')).to.have.text('Test title(completed {completedLessons} of {totalLessons} lessons)');
  });
  ...
  describe('lesson navigation', () => {
    describe('nextLesson', () => {
      it('should not render link when nextLesson is null', () => {
        ...
        expect(wrapper.find(`.${style.lessonNavigation}`)).not.to.have.text('next lesson');
      });
    ...
    describe('prevLesson', () => {
      it('should not render link when prevLesson is null', () => {
        ...
        expect(wrapper.find(`.${style.lessonNavigation}`)).not.to.have.text('previous lesson');
      });
  ...
  describe('teachingMaterials', () => {
    describe('lessonContent === true', () => {
      it('renders list of teachingMaterials', () => {
        ...
        const wrapper = mount(<Lesson {...defaultProps} lesson={lessonWithTeachingMaterial} />, {
          context: { t: translationString => translationString },
        });

        expect(wrapper).to.contain(<Title>Lesson content</Title>);

        expect(wrapper).to.contain(<TeachingMaterial teachingMaterial={teachingMaterial} number={1} />);
      });
    });
    ...
    describe('listingItem === true', () => {
      it('does not include icon if it is empty', () => {
        ...
        expect(wrapper).not.to.contain(<span className={style.lessonHintIcon}>
      ...
      it('renders list of teachingMaterials', () => {
        ...
        expect(wrapper).to.contain(<Title>Lesson links & hints</Title>);
        expect(wrapper).to.contain(<FaIcon icon={lessonHintMaterial.listingIcon} />);

        expect(wrapper).to.contain(<dfn>{lessonHintMaterial.listingTitle}</dfn>);
        expect(wrapper).to.contain(<a href={lessonHintMaterial.link} rel="noopener noreferrer" target="_blank">
          {lessonHintMaterial.link}
                                   </a>);

        expect(wrapper).not.to.contain(<dfn>{notRenderedMaterial.listingTitle}</dfn>);
      });
    });
  });
  ...
  describe('commonMistakes', () => {
    it('renders list of commonMistakes', () => {
      ...
      expect(wrapper.find(`.${style.lessonInfoContainer}`)).to.contain(<Title>Lesson links & hints</Title>);
      expect(wrapper.find(`.${style.lessonInfoContainer}`)).to.contain(<FaIcon icon="exclamation-triangle" />);
      expect(wrapper.find(`.${style.lessonInfoContainer}`)).to.contain(<dfn>Common mistakes</dfn>);

      expect(wrapper.find(`.${style.lessonCommonMistakeProblem}`).first()).to.have.text(commonMistake1.problem);
      expect(wrapper.find(`.${style.lessonCommonMistakeSolution}`).first()).to.have.text(commonMistake1.solution);

      expect(wrapper.find(`.${style.lessonCommonMistakeProblem}`).last()).to.have.text(commonMistake2.problem);
      expect(wrapper.find(`.${style.lessonCommonMistakeSolution}`).last()).to.have.text(commonMistake2.solution);
    });
  });
