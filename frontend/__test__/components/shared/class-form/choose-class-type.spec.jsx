...
describe('<ChooseClassTypeComponent />', () => {
  ...
  it('renders a school_class and an extracurricular button', () => {
    ...
    expect(wrapper.containsMatchingElement(<button>School class</button>)).to.equal(true);
    expect(wrapper.containsMatchingElement(<button>Extracurricular</button>)).to.equal(true);
  });
  ...
  it('sets type to school_class and calls onSubmit on School class button click', (done) => {
    ...
    expect(get(state, 'form.classForm.values.resourceType')).to.equal('school_class');
    setTimeout(() => {
  ...
  it('sets type to extracurricular and calls onSubmit on Extracurricular button click', (done) => {
    ...
    const state = store.getState();
    expect(get(state, 'form.classForm.values.resourceType')).to.equal('extracurricular');
    setTimeout(() => {
      expect(callCount).to.equal(1);
      done();
    }, 0);
