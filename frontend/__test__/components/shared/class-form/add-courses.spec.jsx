...
describe('<AddCourses />', () => {
  ...
  it('renders all topic buttons and courses', () => {
    const wrapper = setupWrapper({ submitText: 'Create class' }).find(AddCourses);

    expect(
      wrapper.containsMatchingElement(<TopicSelection topics={topics} selectedTopic={topics[0]} />),
    ).to.be.true;
  ...
  it('selects course on click', () => {
    ...
    const courseButton = wrapper.find(`.${style.addCourseButtonContainer}`).at(0);
    ...
    expect(wrapper.find(`.${style.addCourseButtonContainer}`).at(0)).to.have.text('added');
  });
  ...
  */
  it.skip('deselects course on click', () => {
    ...
    const courseButton = firstCourseElement.find(`.${style.addCourseButtonContainer}`);
    ...
    expect(courseButton.find(`.${style.addCourseButtonContainer}`)).to.have.text('remove');
  ...
  it('renders other courses on filterButton click', () => {
    ...
    expect(wrapper.containsMatchingElement(<Course course={topics[0].courses[0]} />)).to.equal(
      true,
    );
    expect(wrapper.containsMatchingElement(<Course course={topics[1].courses[0]} />)).to.equal(
      false,
    );

    wrapper
      .find(`.${style.topicFilters} Button`)
      .at(1)
      .simulate('click');
