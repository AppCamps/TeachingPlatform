...
describe('<CreateClassComponent />', () => {
  ...
  it('initialize with state { page: 1 }', () => {
    const wrapper = shallow(
      <CreateClassComponent {...Object.assign({}, defaultProps, { classes: {} })} />,
      {
        context: {
          t(translation) {
            return translation;
  ...
  describe('progressBar', () => {
    ...
    xit('sets done and active classes based on page', () => {
      ...
      expect(wrapper).to.contain(<button className={`${style.progressBarStep} ${style.progressBarStepActive}`}>
      ...
      expect(wrapper).to.contain(<button className={style.progressBarStep}>
        <span className={style.doneIcon}>
          <FaIcon icon="check" />
        </span>
      ...
      expect(wrapper).to.contain(<button className={`${style.progressBarStep} ${style.progressBarStepDone}`}>
        <span className={style.doneIcon}>
          <FaIcon icon="check" />
        </span>
        <span className={style.number}>{1}</span>
    ...
    it('renders ChooseClassTypeComponent', () => {
      ...
      expect(wrapper.find(`.${createClassStyle.chooseClassType}`)).to.be.present();
    });
  });
});
