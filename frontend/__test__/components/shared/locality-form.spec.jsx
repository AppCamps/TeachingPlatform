...
describe('<LocalityForm />', () => {
  ...
  const setupWrapper = (props = {}) => {
    ...
    return mount(component, {
      context: { t(translation) { return translation; } },
      childContextTypes: { t: PropTypes.func },
    });
  };

  const changeInput = (wrapper, name, value) => {
    const field = wrapper.find('Field').filterWhere(c => c.prop('name') === name);

    if (field.length === 0) { throw new Error(`Cannot find input: ${name}`); }
  ...
  describe('fields and submit', () => {
    it('renders form role_teacher', () => {
      ...
      expect(wrapper.containsMatchingElement(
        <Field name="schoolType" label="School type" component={SelectWithLabel} required />,
      )).to.be.true;
      ...
      expect(wrapper.containsMatchingElement(
        <Field name="country" label="Country" component={SelectWithLabel} required />,
      )).to.be.true;
  ...
  describe('Form Validations', () => {
    ...
    it('calls createLocality for submitted forms', () => {
      const selectedCountry = factory.build('country', { postalCodeFormat: '/^[0-9A-z]{5}$/' });
      ...
      Object.keys(expectedValues).forEach(key => changeInput(wrapper, key, expectedValues[key]));
