...
describe("<Classes />", () => {
  ...
  it("lists classes", () => {
    ...
    expect(classContainers.at(0).find(`.${style.title}`)).to.have.text(
      klass.title
    );
    expect(classContainers.at(0).find(`.${style.identifier}`)).to.have.text(
      klass.identifier
    );
  });
  ...
  it("has button to start courses, if no session marked as completed", () => {
    ...
    expect(continueLink.find(Link)).to.have.prop(
      "to",
      `/classes/${klass.id}/some/link`
    );
  });
  ...
  it("has button to continue courses, if at least one session is completed", () => {
    ...
    expect(continueLink.find(Link)).to.have.prop(
      "to",
      `/classes/${klass.id}/someCourse/link`
    );
  });
  ...
  it("has edit button and shows no courses message", () => {
    ...
    expect(wrapper.find(`.${style.noCourses}`)).to.have.text(
      "You did not add any courses yet"
    );
  });
  ...
  it('renders courses with Lessons, lesson progress with a "not yet started" message and proper class-lesson link', () => {
    ...
    const course = factory.build(
      "course",
      {
        ...
        topic: factory.build(
          "topic",
          {},
          { coursesCount: 0, preparationMaterialsCount: 0 }
        ),
      },
      { lessonsCount: 0 }
    );
    ...
    expect(renderedLesson.find(Link)).to.have.prop(
      "to",
      `/classes/${klass.id}/another/testCase`
    );
    expect(renderedLesson).to.have.prop("isCompleted", false);

    expect(wrapper.find(`p.${style.helpText}`)).to.contain.text(
      "Not yet started"
    );
    ...
    const course = factory.build(
      "course",
      {
        ...
        topic: factory.build(
          "topic",
          {},
          { coursesCount: 1, preparationMaterialsCount: 0 }
        ),
      },
      { lessonsCount: 0 }
    );
