...
describe("<CourseList/>", () => {
  ...
  it("should have given props", () => {
    const wrapper = mount(<CourseList {...defaultProps} />, {
      context: { t: (t) => t },
    });
  ...
  it("should render course lists with lessons", () => {
    ...
    expect(
      wrapper.containsMatchingElement(
        <Course course={courses[0]} topic={topic} />
      )
    ).to.eql(true);
    expect(
      wrapper.containsMatchingElement(
        <Course course={courses[1]} topic={topic} />
      )
    ).to.eql(true);
  });
  ...
  it("should give correct lessonUrl prop", () => {
    ...
    const lessonUrlFn = wrapper.find(Course).first().prop("lessonUrl");
    const fnParams = { lesson: { slug: "test" } };

    expect(lessonUrlFn(fnParams)).to.eql(
      `/topics/${topic.slug}/${courses[0].slug}/test`
    );
  });
});
