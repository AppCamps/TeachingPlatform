...
describe('<Preparations />', () => {
  ...
  it('renders preparationMaterials for selectedTopic', () => {
    const preparationMaterial = factory.build('preparationMaterial', { topic: topicHtml });
    const wrapper = mount(
      <Preparations {...{ ...defaultProps, preparationMaterials: [preparationMaterial] }} />,
      { context: { t: translationString => translationString } },
    );

    expect(wrapper).to.contain(<PreparationMaterial preparationMaterial={preparationMaterial} />);
  });
});
