...
describe('<CourseLesson/>', () => {
  const topic = factory.build('topic', { coursesCount: 0, preparationMaterialsCount: 0 });
  ...
  it('should should give right lessonUrl parameter', () => {
    const wrapper = mount(<CourseLesson {...defaultProps} topicSlug="topic-test" />, {
      context: { t: translationString => translationString },
    });
    const { lessonUrl } = wrapper.find(Lesson).props();

    expect(lessonUrl({ slug: 'test' })).to.equal(`/topics/topic-test/${course.slug}/test`);
  });
  ...
  it('should have props given props', () => {
    const wrapper = mount(<CourseLesson {...Object.assign({}, defaultProps)} />, {
      context: { t: translationString => translationString },
    });
