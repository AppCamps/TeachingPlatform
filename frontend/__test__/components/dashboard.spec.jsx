...
describe('<Dashboard/>', () => {
  it('should have navigation links', () => {
    const wrapper = shallow(<Dashboard location={{ pathname: '/' }} />, { context: { t: v => v } });
  ...
  it('displays activityIndicator for /posts if unreadPostsPresent is true', () => {
    const wrapper = shallow(<Dashboard location={{ pathname: '/' }} unreadPostsPresent />, {
      context: { t: v => v },
    });

    expect(wrapper.find('Link[to="/posts"]').find('.activityIndicator')).to.have.length(1);
  });

  it('does not display activityIndicator if unreadPostsPresent is false', () => {
    const wrapper = shallow(<Dashboard location={{ pathname: '/' }} unreadPostsPresent={false} />, {
      context: { t: v => v },
    });
