...
describe("<LoginForm />", () => {
  ...
  describe("login error", () => {
    it("translates errors for invalid credentials or login expire", () => {
      ...
      let wrapper = setupWrapper(
        <LoginForm
          {...defaultProps}
          authenticationError={invalidPasswordError}
        />
      );

      expect(wrapper.find(`.${style.loginError}`)).to.have.text(
  ...
  it("disables inputs and buttons while submitting", () => {
    ...
    const wrapper = setupWrapper(
      <LoginForm {...defaultProps} loginUser={() => promise} />
    );
    ...
    wrapper
      .find("input")
      .forEach((input) => expect(input.prop("disabled")).to.equal(true));
