...
describe('<ClassLesson />', () => {
  ...
  it('should should give right lessonUrl parameter', () => {
    ...
    expect(lessonUrl({ slug: 'test' })).to.equal(`/classes/${klass.id}/${course.slug}/test`);
  });
  ...
  describe('actions', () => {
    ...
    it('should have complete lesson button and call markLessonAsComplete on click', () => {
      ...
      const wrapper = mount(
        <ClassLesson {...defaultProps} markLessonAsComplete={markLessonAsCompleteMock} />,
        { context: { t: translationString => translationString } },
      );

      const button = wrapper.find(`.${style.completeLessonButton}`).find(Button);
    ...
    it('should have incomplete lesson link and call markLessonAsIncomplete on click', () => {
      ...
      expect(completedLesson.childAt(0)).to.have.text('This lesson was tagged as completed.');
      expect(completedLesson.find('a')).to.have.text('Undo');

      completedLesson.find('a').simulate('click');

      expect(markLessonAsIncompleteMock.mock.calls[0]).to.eql([newClass, lesson]);
    });
  ...
  it('should have props given props', () => {
    ...
    expect(wrapper.prop('markLessonAsIncomplete')).to.equal(markLessonAsIncomplete);
  });
});
