...
class ORMFactory {
  ...
  buildMany(
    ...
    for (let i = 0; i < count; i += 1) {
      relationInstances.push(
        this.build(factoryName, customAttributes, buildModifiers)
      );
    }
  ...
  createMany(
    ...
    for (let i = 0; i < count; i += 1) {
      relationInstances.push(
        this.create(factoryName, customAttributes, buildModifiers)
      );
    }
  ...
  buildList(factoryName, count, customAttributes = {}, buildModifiers = {}) {
    if (!count) {
      throw new Error(
        `You must specify a count parameter for buildList('${factoryName}')`
      );
    }
    const list = [];
    for (let i = 0; i < count; i += 1) {
  ...
  createList(factoryName, count, customAttributes = {}, buildModifiers = {}) {
    if (!count) {
      throw new Error(
        `You must specify a count parameter for createList('${factoryName}')`
      );
    }
    const list = [];
    for (let i = 0; i < count; i += 1) {
