...
class EditUser extends Component {
  @autobind
  onSubmit(formValues) {
    const { updateUser, user: { id }, initialize } = this.props;
  ...
  handleEmailConfirmationRequestLinkClick() {
    const { emailConfirmationRequest, user: { unconfirmedEmail } } = this.props;
    return emailConfirmationRequest({ email: unconfirmedEmail });
  }

  editUserForm() {
    const { t } = this.context;
    const { submitting, handleSubmit, user, pristine, submitSucceeded } = this.props;

    let unconfirmedInfo = null;
    if (user.unconfirmedEmail) {
      unconfirmedInfo = t('{email} is still unconfirmed. {emailConfirmationRequestLink}', {
        email: <b>{user.unconfirmedEmail}</b>,
        emailConfirmationRequestLink: (
          <a tabIndex="0" onClick={this.handleEmailConfirmationRequestLinkClick}>
    ...
    return (
      <form onSubmit={handleSubmit(this.onSubmit)}>
        ...
        <div className={style.actions}>
          ...
          <Button isAction disabled={submitting} type="submit" leftIcon={submitIcon}>
            {buttonText}
          </Button>
        </div>
      </form>
