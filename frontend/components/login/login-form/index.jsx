...
class LoginForm extends Component {
  ...
  errorMessage() {
    ...
    const errorMessage =
      errorCodes[authenticationError.code] || authenticationError.title;
    ...
    if (authenticationError.code === "email_unconfirmed") {
      errorAction = (
        <a
          ...
          role="button"
          onClick={() =>
            this.handleEmailConfirmationRequestLinkClick(
      ...
      <div className={style.links}>
        <div>
          {t("Did not sign up yet? {registrationLink}", {
            registrationLink: (
              <Link to={registrationPath}>{t("Register here")}</Link>
            ),
          })}
        </div>
        <div>
          {t("Did not receive a confirmation email? {confirmationLink}", {
            confirmationLink: (
              <Link to={confirmationPath}>{t("Click here")}</Link>
            ),
          })}
        </div>
      </div>
