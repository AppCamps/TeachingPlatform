...
const passwordResetRequestSuccess =
  t('If the email you provided is registered with App Camps you will soon receive an email containing a link to the reset form.');

const passwordResetSuccess =
  t('Password successfully changed. You can now login with your new password.');
const passwordResetFailureInvalid =
  t('There was an error with your reset link. Please restart the password recovery process.');
const passwordResetFailureExpired =
  t('Your password reset link has expired. Please restart the password recovery process.');
...
export function passwordReset(formData, passwordResetToken) {
  return dispatch => (
    passwordResetApiCall(formData, passwordResetToken).then(
      () => {
        dispatch(push('/'));
        dispatch(
          requestNotification({
      ...
      if (error instanceof SubmissionError) {
        if (error.errors.resetPasswordToken) {
          ...
          if (error.errors.resetPasswordToken === 'has expired, please request a new one') {
